...
<main class="gv" aria-label="Consultation des notes">
    ...
    <section class="gv-panel" aria-label="Filtres">
        ...
        <div class="gv-step gv-step--locked" id="gv-step-class">
            <div class="gv-step-head">
                ...
                <div class="gv-step-info">
                    ...
                    <div class="gv-step-hint">Choisissez la classe — les résultats se chargent automatiquement</div>
                </div>
        ...
        <div class="gv-field--sem">
            <span class="gv-label">
                ...
                Semestre — le changement recharge automatiquement
            </span>
            <div class="gv-semtabs" role="tablist" aria-label="Choix du semestre">
                <button type="button" class="gv-tab is-active" data-value="1"
                        aria-pressed="true" id="sem-btn-1">
                    <span>S1</span><small>Semestre 1</small>
                </button>
                <button type="button" class="gv-tab" data-value="2"
                        aria-pressed="false" id="sem-btn-2">
        ...
        {{-- Barre de recherche (affichée après chargement) --}}
    ...
    <div id="gv-loading" style="display:none;">
        <div class="gv-loading-card">
            <div class="gv-loading-spin"></div>
            <span>Chargement des notes…</span>
        </div>
    </div>

    {{-- ===== ZONE DE RÉSULTATS ===== --}}
    {{-- Ce div est la cible du fetch JS ET du rendu serveur initial.
         Le JS extrait son contenu depuis la réponse HTML de /note?... --}}
    ...
    @if($classroom && isset($studentsData) && $studentsData->isNotEmpty())
        ...
        @php
            ...
            $calculerRangs = function(array $moyennes) use ($ordinalFr): array {
                ...
                arsort($avecNote);
                $rangs = []; $rangDebut = 1; $prevMoy = null; $nbDansGroupe = 0;
                foreach ($avecNote as $sid => $moy) {
                    if ($prevMoy !== null && $moy == $prevMoy) { $nbDansGroupe++; }
                    else { $rangDebut += $nbDansGroupe; $nbDansGroupe = 1; $prevMoy = $moy; }
                    $rangs[$sid] = $ordinalFr($rangDebut);
                }
                foreach (array_keys(array_filter($moyennes, fn($m) => $m === null)) as $sid) { $rangs[$sid] = '—'; }
                return $rangs;
            };
            ...
            $rangsAnnuelsVue = []; $moyAnnuellesVue = [];
            if ($currentSemester == 2) {
                foreach ($studentsData as $sRow) {
                    $sid = $sRow['student']->id;
                    $rec = $sRow['student']->recordings->where('year_id', request('year_id'))->where('classroom_id', request('classroom_id'))->first();
                    $ms1 = null;
                    if ($rec) {
                        $nS1s = \App\Models\Note::with(['ratio'])->where('recording_id', $rec->id)->where('semester', 1)->get();
                        $tp=0; $tc=0;
                        foreach ($nS1s as $nS1) {
                            ...
                            if ($m !== null) { $c = $nS1->ratio->coefficient ?? 1; $tp += $m * $c; $tc += $c; }
                        }
        ...
        <section class="gv-banner" aria-label="Résumé classe">
            ...
            <div class="gv-stats" role="list">
                <div class="gv-stat" role="listitem"><span class="gv-stat__k">Élèves</span><span class="gv-stat__v">{{ $totalEleves }}</span></div>
                <div class="gv-stat" role="listitem"><span class="gv-stat__k">Admis</span><span class="gv-stat__v is-ok">{{ $admis }}</span></div>
                <div class="gv-stat" role="listitem"><span class="gv-stat__k">Ajournés</span><span class="gv-stat__v is-bad">{{ $totalEleves - $admis }}</span></div>
                ...
                <div class="gv-stat" role="listitem"><span class="gv-stat__k">Matières</span><span class="gv-stat__v">{{ $subjects->count() }}</span></div>
            </div>
        </section>
                        ...
                        <summary class="gv-summary">
                            ...
                            <div class="gv-kpis">
                                ...
                                <div class="gv-kpi">
                                    ...
                                    <span class="gv-kpi__v is-brand">{{ $rangSem !== '—' ? $rangSem.'/'.$totalEleves : '—' }}</span>
                                </div>
                                @if($currentSemester == 2)
                                    <div class="gv-kpi">
                                    ...
                                    <div class="gv-kpi">
                                        ...
                                        <span class="gv-kpi__v is-brand">{{ $rangAnn !== '—' ? $rangAnn.'/'.$totalEleves : '—' }}</span>
                                    </div>
                                @endif
                            </div>
                                        ...
                                        @foreach($subjects as $subjectId => $subjectInfo)
                                            ...
                                            <tr>
                                                ...
                                                <td data-label="Interros" class="t-center">
                                                    @if(!empty($interros))
                                                        <div class="gv-chips">@foreach($interros as $iv)<span class="gv-chip {{ $iv >= 10 ? 'is-ok' : 'is-bad' }}">{{ number_format($iv, 2) }}</span>@endforeach</div>
                                                    @else <span class="gv-dash">—</span> @endif
                                                </td>
                                                <td data-label="Moy. I" class="t-center">
                                                    @if($moyInterros !== null)<span class="gv-mark {{ $moyInterros >= 10 ? 'is-ok' : 'is-bad' }}">{{ number_format($moyInterros, 2) }}</span>
                                                    @else <span class="gv-dash">—</span> @endif
                                                </td>
                                                <td data-label="D1" class="t-center">
                                                    @if($note?->devoir1 !== null)<span class="gv-mark {{ $note->devoir1 >= 10 ? 'is-ok' : 'is-bad' }}">{{ number_format($note->devoir1, 2) }}</span>
                                                    @else <span class="gv-dash">—</span> @endif
                                                </td>
                                                <td data-label="D2" class="t-center">
                                                    @if($note?->devoir2 !== null)<span class="gv-mark {{ $note->devoir2 >= 10 ? 'is-ok' : 'is-bad' }}">{{ number_format($note->devoir2, 2) }}</span>
                                                    @else <span class="gv-dash">—</span> @endif
                                                </td>
                                                <td data-label="Moy /20" class="t-center">
                                                    @if($moy20 !== null)<span class="gv-final {{ $moy20 >= 10 ? 'is-ok' : 'is-bad' }}">{{ number_format($moy20, 2) }}</span>
                                                    @else <span class="gv-dash">—</span> @endif
                                                </td>
                                                <td data-label="Coeff" class="t-center"><span class="gv-coef">{{ $coef }}</span></td>
                                                <td data-label="Moy×C" class="t-center">
                                                    @if($moyCoeff !== null)<span class="gv-mark {{ $moyCoeff >= 10 * $coef ? 'is-ok' : 'is-warn' }}">{{ number_format($moyCoeff, 2) }}</span>
                                                    @else <span class="gv-dash">—</span> @endif
                                                </td>
                                            </tr>
                                        @endforeach
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $$('.gv-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Recharger si une classe est déjà choisie
    ...
    async function loadActiveYear() {
        try {
            ...
            renderOpts('sector', [], 'Erreur : ' + e.message);
        }
    }
    ...
    async function loadSectors() {
        ...
        try {
            ...
            setLoading('sector', false);
            renderOpts('sector', [], 'Erreur de chargement.');
        }
    }
    ...
    async function onClassroom(c, auto = false) {
        ...
        state.classLabel  = c.name;
        setBc('class', c.name);
        // Chargement automatique dès la classe choisie
        await loadResults();
    }

    // ── Chargement des résultats (fetch HTML partiel) ────────
    async function loadResults() {
        if (!state.classroomId || !state.yearId) return;

        $('gv-loading').style.display          = 'flex';
        $('gv-results-zone').style.opacity      = '0.3';
        $('gv-results-zone').style.pointerEvents = 'none';
        ...
        try {
            ...
            if (hasList) {
                ...
                $('gv-subtitle').textContent =
                    `${state.classLabel} · Semestre ${state.semester} · ${state.yearLabel}`;
                initSearch();
                initAccordions();
            } else {
            ...
            showEmpty('Erreur de chargement', e.message);
        } finally {
            $('gv-loading').style.display           = 'none';
            $('gv-results-zone').style.opacity       = '1';
            $('gv-results-zone').style.pointerEvents = '';
        }
    }

    function showEmpty(title, msg) {
        $('gv-results-zone').innerHTML = `
            <section class="gv-empty">
                <div class="gv-empty__icon"><i class="fas fa-book-open"></i></div>
                <h3>${escHtml(title)}</h3>
                <p>${escHtml(msg)}</p>
    ...
    function initSearch() {
        ...
        // Cloner l'input pour retirer les anciens listeners
        ...
        newInput.addEventListener('input', () => {
            ...
            if (countBox) countBox.textContent = q
    ...
    function initAccordions() {
        ...
        // Rebrancher le bouton "Tout ouvrir / fermer"
    ...
    function renderOpts(step, items, emptyMsg, onClick) {
        ...
        if (!container) return;
        if (!items.length) {
            container.innerHTML = `<div class="gv-ph gv-ph--warn">${escHtml(emptyMsg)}</div>`;
            return;
        }
        container.innerHTML = items.map(item => `
            <button type="button" class="gv-opt" data-id="${item.id}">
    ...
    function resetFrom(step) {
        ...
        for (let i = idx; i < steps.length; i++) {
            ...
            if (opts) opts.innerHTML = `<div class="gv-ph">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12l7 7 7-7"/></svg>
                Choisissez d'abord ${s === 'promo' ? 'une filière' : 'une promotion'}
            </div>`;
            const bcLbl  = $(`gv-bc-${s}-lbl`);  if (bcLbl)  bcLbl.textContent  = s === 'promo' ? 'Promotion' : 'Classe';
            const bcItem = $(`gv-bc-${s}`);        if (bcItem) bcItem.classList.remove('active');
...
{{-- ===== STYLES ===== --}}
{{-- Le bloc @section('content') contient également le HTML des résultats
     qui est rendu côté serveur via la méthode index() du contrôleur.
     Le JS remplace dynamiquement #gv-results-zone par le contenu
     extrait de la réponse HTML de la même route /note?... --}}

{{-- Les styles du tableau des résultats sont injectés ici
     pour qu'ils soient disponibles même si le contenu est chargé dynamiquement --}}
<style>
:root{
  --bg:#f3f5f9;--card:#fff;--muted:#6b7280;--text:#0f172a;--border:#e6eaf0;
...
.gv-subtitle{margin:2px 0 0;font-size:.9rem;color:var(--muted);transition:color .3s}
.gv-topbar__right{display:flex;gap:10px;flex-wrap:wrap}
.gv-btn{border:1px solid var(--border);background:var(--card);color:var(--text);border-radius:12px;height:42px;padding:0 14px;cursor:pointer;display:inline-flex;align-items:center;gap:8px;font-weight:700;transition:transform var(--tr),box-shadow var(--tr)}
.gv-btn:hover{transform:translateY(-1px);box-shadow:var(--shadow)}
...
.gv-ph--warn{color:var(--warn);font-style:normal}
...
/* Semestre */
.gv-field--sem{display:flex;flex-direction:column;gap:6px}
...
.gv-input{border:none;background:transparent;outline:none;width:100%;font-size:.9rem;height:40px}
.gv-minihelp{color:var(--muted);font-weight:800;font-size:.84rem}

/* Loading */
#gv-loading{display:flex;justify-content:center}
.gv-loading-card{display:flex;align-items:center;gap:12px;background:var(--card);border:1px solid var(--border);border-radius:16px;padding:16px 24px;box-shadow:var(--shadow);color:var(--muted);font-weight:700}
...
.gv-chip{font-family:var(--mono);font-weight:1000;font-size:.72rem;padding:3px 8px;border-radius:999px;border:1px solid var(--border)}
